...
public class ReportValidator {
    ...
    public void validateCloseTimeReport(CloseDate closeDate) {
        userRepository.findById(closeDate.getIdUser()).orElseThrow(() -> new ValidationException("user.not.found"));
    }

    public void validateOpenTimeReport(CloseDate closeDate) {
        UserPO userPO = userRepository.findById(closeDate.getIdUser()).orElseThrow(() -> new ValidationException("user.not.found"));
