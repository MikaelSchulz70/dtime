...
class FixRateTableRow extends React.Component {
    ...
    delete() {
        ...
        const self = this;
        const service = new FixRateService();
    ...
    render() {
        ...
        var customerRate = (this.state.rate.rate == null ? '' : this.state.rate.rate);
        ...
        return (
            <tr>
                <td>
                    <input name="rate" type="text" value={customerRate} maxLength="10" onChange={this.handleRateChange} />
                </td>
                <td>
                    <DatePicker
                        type="date"
...
class FixRatesTable extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        this.state.rates.forEach(function (rate) {
            rows.push(
                <FixRateTableRow rate={rate} key={rate.id} refresh={self.props.refresh} />);
        });
...
export default class ProjectFixRates extends React.Component {
    constructor(props) {
        super(props);
        ...
        const idProject = this.props.match.params.projectId;
    ...
    backToOverview() {
        this.props.history.push('/fixrates');
    }

    loadFromServer() {
        const self = this;
        var service = new FixRateService();
        service.getRatesForProject(this.state.idProject)
            .then(response => {
    ...
    render() {
        ...
        if (this.state.rates != null && this.state.rates.length > 0) {
            header = this.state.rates[0].companyName + ', ' + this.state.rates[0].projectName;
        }
        ...
        return (
            <div className="container">
                ...
                <div className="row">
                    <FixRatesTable rates={this.state.rates} refresh={refresh} />
                </div>
                <div className="row">
