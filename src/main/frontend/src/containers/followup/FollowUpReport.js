        ...
        this.state.report.followUpCategoryReports.forEach(function (followUpCategoryReport) {
            rows.push(
                <tr key={key} className="bg-success text-white">
                    <th>{followUpCategoryReport.projectCategory}</th>
                    <th>User</th>
                    <th>Hours</th>
                    <th>Total amount</th>
...
class FollowUpCompanyReportTable extends React.Component {
    constructor(props) {
        super(props);
        this.state = { report: this.props.report };
    }
    ...
    render() {
        ...
        var key = 1;
        this.state.report.followUpCompanyReports.forEach(function (followUpCompanyReport) {
            rows.push(
                <tr key={key}>
                    <td>{followUpCompanyReport.companyName}</td>
                    <td>{followUpCompanyReport.totalHours}</td>
                    <td>{followUpCompanyReport.totalAmount}</td>
                    <td>{followUpCompanyReport.amountSubcontractor}</td>
                    <td>{followUpCompanyReport.amount}</td>
                    <td>{followUpCompanyReport.comment}</td>
                </tr>);
            key++;
        });
        ...
        return (
            <div className="container-fluid ml-4">
                <div className="row mb-3">
                    ...
                    <div className="col-sm-3">
                        <select className="form-control input-sm" value={this.state.reportType} name="reportType" onChange={this.typeChange}>
                            <option value={Constants.FOLLOW_UP_CATEGORY}>Project category</option>
                            <option value={Constants.FOLLOW_UP_USER}>User</option>
                            <option value={Constants.FOLLOW_UP_COMPANY}>Company</option>
                        </select>
                    </div>
                ...
                <div className="row">
                    ...
                    ) : this.state.reportType === Constants.FOLLOW_UP_COMPANY ? (
                        <FollowUpCompanyReportTable report={this.state.report} reportView={this.state.reportView} fromDate={this.state.report.fromDate} />
                    ) : ''}
                </div>
            </div>
        );
